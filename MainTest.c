...
#include "MainTest.h"
int iProgramCount_g = 0;
PROGRAM_INFO astProgramInfo_g[PROGRAM_MAX] = {0};
/*********************************************************************************************************************************
...
int ParseTransportStream(FILE *pfTsFile)
{
	...
	iProgramCount_g = ParsePAT_Table(pfTsFile, iTsPosition, iTsLength, astPAT_Info); //PAT
	if (iProgramCount_g <= 0) //no PAT table in the transport stream no use do the next
	{
		return -1;
	}
	...
	if (-1 == ParseAllProgramPMT(pfTsFile, iTsPosition, iTsLength, astPAT_Info, iProgramCount_g, astPMT_Info)) //PMT
	{
		return -1;
	}

	CombinationAllInfo(astProgramInfo_g, iProgramCount_g, astPAT_Info, astPMT_Info, astSDT_Info);
