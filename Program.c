	...
	for (iLoopTime = 0; iLoopTime < iProgramCount; iLoopTime++)
	{
		for (iStreamIndex = 0; iStreamIndex < iProgramCount; iStreamIndex++)
		{
			if (pstProgramInfo[iLoopTime].uiProgramNumber == pstPMT_Info[iStreamIndex].uiProgramNumber)
			{
				...
				pstProgramInfo[iLoopTime].uiPCR_PID = pstPMT_Info[iStreamIndex].uiPcrPID;
			}
		}
	}
}
...
void PrintAllProgramInfo(PROGRAM_INFO *pstProgramInfo, int iProgramCount)
{
	...
	//qDebug("ProgramId PMT_PID  PCR_PID  FreeCA  ProgramName           VideoPID  AudioPID1 AudioPID2\n");
	printf("ProgramId\tPMT_PID\tPCR_PID\tProgramName\tVideoPID\tAudioPID1\tAudioPID2\n");
	for (iProgramIndex = 0; iProgramIndex < iProgramCount; iProgramIndex++)
	{
		printf("%5d\t%5d\t%5d\t%-25s\t%5d\t%5d\t%5d\n",
			pstProgramInfo[iProgramIndex].uiProgramNumber,
			pstProgramInfo[iProgramIndex].uiPMT_PID,
			pstProgramInfo[iProgramIndex].uiPCR_PID,
			pstProgramInfo[iProgramIndex].aucProgramNanme,
			pstProgramInfo[iProgramIndex].uiVideoPID,
			pstProgramInfo[iProgramIndex].auiAudioPID[0],
